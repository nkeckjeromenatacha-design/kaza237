...
/* ============================================================
   IMPORTS — TOUT EN HAUT obligatoirement
   ============================================================ */
...
/* ============================================================
   1. DONNÉES DES LOGEMENTS (exemples)
   ============================================================ */
const logementsData = [
  {
    id: 'studio1',
...
function afficherLogements(logements, nombre) {
  ...
  logements.slice(0, nombre).forEach(function(logement) {
    ...
    const equipements = (logement.equipements || []).map(function(eq) {
      ...
      return `<span><i class="fa-solid ${icones[eq] || 'fa-check'}"></i> ${eq}</span>`;
    }).join('');
    ...
    const btnReserver = estDisponible
      ? `<button class="btn-reserver"
                 onclick="preRemplirFormulaire('${logement.titre}', '${typeAffiche}', '${logement.prixAffiche}', '${villeAffiche}, ${logement.quartier}')">
    ...
    container.innerHTML += `
      <div class="carte-logement">
        ...
        <div class="carte-logement-corps">
          ...
          <div class="logement-stats">
            <span><i class="fa-solid fa-eye"></i> ${logement.vues || 0} vues</span>
            <span><i class="fa-solid fa-heart"></i> ${logement.likes || 0} j'aime</span>
          </div>
...
/* ============================================================
   3. CHARGER DEPUIS FIREBASE
   ============================================================ */
async function chargerLogementsFirebase() {
  try {
    const q = query(
      collection(db, 'properties'),
      where('type', 'in', ['studio-meuble', 'chambre-meuble', 'appartement-meuble']),
...
function filtrerReservations() {
  ...
  afficherLogements(logementsFiltres, logementsAffiches);
  document.querySelector('.section-reservations').scrollIntoView({ behavior: 'smooth' });
}
...
/* ============================================================
   ...
   ============================================================ */
function preRemplirFormulaire(titre, type, prix, lieu) {
  ...
  if (inputMessage) {
    inputMessage.value = 'Je souhaite réserver : ' + titre + ' à ' + lieu;
  }

  document.querySelector('.section-formulaire-resa').scrollIntoView({ behavior: 'smooth' });
}


/* ============================================================
   6. SOUMETTRE LE FORMULAIRE → FIREBASE + WHATSAPP
   ============================================================ */
async function soumettreReservation(event) {
  ...
  const lieu = message.includes(' à ') ? message.split(' à ')[1] : '';

  /* Sauvegarder dans Firebase */
  try {
    await addDoc(collection(db, 'reservations'), {
      prenom, nom, telephone, type, dates, budget, message,
      statut    : 'en_attente',
      createdAt : new Date()
    });
  } catch (error) {
...
window.filtrerParCategorieResa = filtrerParCategorieResa;
window.reinitialiserFiltresResa = reinitialiserFiltresResa;
window.chargerPlusDeLogements  = chargerPlusDeLogements;
window.afficherChampsDates     = afficherChampsDates;
